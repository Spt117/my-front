...
//     const { variant, shopifyBoutique } = useShopifyStore();
//     const { handleCopy } = useCopy();

//     const shopifyUrls = variant?.ids.map((d) => {
//         const boutique = boutiqueFromDomain(d.shop);
//         return {
//             urlShopify: `https://${shopifyBoutique.domain}/admin/products/${d.idProduct?.replace("gid://shopify/Product/", "")}`,
...
//     const classIds =
//         "flex items-center gap-1 font-mono text-xs text-gray-600 ml-1 cursor-pointer transition-transform duration-500 ease-out active:scale-93 flex-1 min-w-0 overflow-hidden"; // <= key
...
//                             className="text-blue-500 hover:underline flex items-center gap-2 w-fit"
//                         >
