                            ...
                            <div className="flex flex-col min-w-0">
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mb-1">
                                    ...
                                    {hiddenPreorderCount > 0 && (
                                        <span
                                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700 border border-orange-200 uppercase tracking-wider"
                                            title={`${hiddenPreorderCount} précommande(s) en attente pour ce client`}
                                        >
                                            <CalendarClock size={10} />+{hiddenPreorderCount} préco
                                        </span>
                                    )}
                                </div>
                    ...
                    {!isExpanded && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 mt-2">
                            {order.lineItems.edges.map(({ node }, index) => {
                                ...
                                const imageUrl = node.variant?.image?.url || node.variant?.product.featuredImage?.url;
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col min-w-0 flex-1">
                                            <h4 className="text-xs font-semibold text-gray-900 truncate group-hover:text-blue-600 transition-colors leading-snug">{node.title}</h4>
                                            {showVariantTitle && <span className="text-[10px] text-purple-600 font-medium truncate">{variantTitle}</span>}
