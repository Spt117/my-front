...
export default function ClientCompact({ client }: { client: ShopifyCustomer }) {
    ...
    const handleEmailClick = (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        handleCopy(client.email || '');
    };

    return (
        <div className="container mx-auto px-4 py-0.5">
            <Link href={clientUrl} className="block group">
                <Card className="overflow-hidden border-0 shadow-sm bg-white/60 backdrop-blur-xl ring-1 ring-black/5 hover:bg-white/80 hover:shadow-md hover:ring-blue-200 transition-all duration-300 cursor-pointer">
                    <div className="px-4 py-3 flex items-center justify-between gap-4">
                        {/* Client Info */}
                        <div className="flex items-center gap-4 min-w-0">
                            <div className="relative shrink-0">
                                <div className="bg-blue-50 text-blue-600 rounded-full p-2 w-10 h-10 flex items-center justify-center border border-blue-100 shadow-sm group-hover:bg-blue-100 group-hover:border-blue-200 transition-all duration-300">
                                    <User size={20} />
                                </div>
                            </div>

                            <div className="flex flex-col min-w-0">
                                <h3 className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition-colors flex items-center gap-2 truncate">
                                    {client.firstName} {client.lastName}
                                    <ArrowRight className="w-3 h-3 text-gray-300 opacity-0 -translate-x-1 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0 group-hover:text-blue-600" />
                                </h3>
