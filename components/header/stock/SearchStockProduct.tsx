...
export default function SearchStockProduct() {
    ...
    useEffect(() => {
        const result = variants.filter((variant) => variant.title.toLowerCase().includes(searchTerm.toLowerCase()) || variant.sku.toLowerCase().includes(searchTerm.toLowerCase()));
        setVariantsFilter(result);
    }, [searchTerm]);

    return (
        <div className="relative w-full">
            <Input disabled={false} type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Produit" className="w-full rounded-lg border-gray-200 focus:ring-2 focus:ring-blue-500 transition-all pr-10" />
            {searchTerm && (
