...
import { ColumnInfo, updateTableRows, fetchTableRow } from "../../actions";
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl overflow-hidden shadow-xl">
                <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                    ...
                    <Button 
                        ...
                        className="bg-gray-800/50 border-gray-700 hover:bg-gray-700 h-9"
                    >
                ...
                <div className="divide-y divide-gray-800/50">
                    {visibleColumns.map((col) => {
                        ...
                        return (
                            <div key={col.name} className={`p-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-start transition-colors ${isEdited ? 'bg-emerald-500/5' : 'hover:bg-gray-800/20'}`}>
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {isEdited && (
                                            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Modifié" />
                                        )}
                                    </div>
                                    ...
                                    {isId ? (
                                        ...
                                        <div className="relative group rounded-lg overflow-hidden border border-gray-700 bg-gray-950 focus-within:border-emerald-500/50 transition-all shadow-inner">
                                            ...
                                            {isEdited && (
                                                <div className="absolute top-10 right-3 flex gap-1 z-10 animate-in fade-in slide-in-from-right-2 duration-200">
                                                    <button onClick={() => handleKeyDown({ key: 'Enter', shiftKey: true, preventDefault: () => {} } as any, col.name, col.type, true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold rounded-md shadow-2xl transition-all hover:scale-105 active:scale-95" title="Sauvegarder (Shift+Entrée)">
                                                        <IconCheck className="w-3.5 h-3.5" />
                                                        SAUVER
                                                    </button>
                                                    <button onClick={() => cancelEdit(col.name)} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-medium rounded-md shadow-2xl transition-all" title="Annuler (Echap)">
                                        ...
                                        <div className="relative group flex items-center gap-2">
                                            <Input 
                                                ...
                                                className={`bg-gray-950 text-gray-100 transition-all ${isEdited ? 'border-emerald-500/50 ring-2 ring-emerald-500/10' : 'border-gray-700 focus:border-emerald-500'}`}
                                                placeholder={`Entrer ${col.name}...`}
                                            />
                                            {isEdited && (
                                                <div className="flex gap-1 shrink-0">
                                                    <button onClick={() => handleKeyDown({ key: 'Enter', preventDefault: () => {} } as any, col.name, col.type, false)} className="p-2 bg-emerald-500 hover:bg-emerald-400 text-white rounded shadow-lg transition-colors" title="Sauvegarder (Entrée)">
                                                        <IconCheck className="w-4 h-4" />
                                                    </button>
                                                    <button onClick={() => cancelEdit(col.name)} className="p-2 bg-gray-800 hover:bg-gray-700 text-gray-400 rounded shadow-lg transition-colors" title="Annuler (Echap)">
