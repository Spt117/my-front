...
async function discoverTablesViaSql(): Promise<string[]> {
  try {
    ...
    if (!error && data && Array.isArray(data)) {
      return data
        .map(
          (t: { tablename?: string; table_name?: string }) =>
  ...
  try {
    ...
    if (adminKey && supabaseUrl) {
      ...
      if (!schemaError && schemaData && schemaData.length > 0) {
        ...
        return {
          tables: tables.sort((a: TableInfo, b: TableInfo) =>
    ...
    return {
      tables: [],
      error:
        "Impossible de récupérer la liste des tables. Vérifiez que SUPABASE_SERVICE_ROLE_KEY est défini dans votre .env ou que vos tables 'x_products', etc. existent.",
    };
  } catch (error) {
...
export async function fetchTableData(
  tableName: string
): Promise<{ data: TableData | null; error: string | null }> {
  try {
...
export async function updateTableRows(
  ...
  try {
    ...
    const failures = results
      .filter((r) => !r.success)
      .map((r) => ({ rowId: r.rowId, error: r.error || "" }));
