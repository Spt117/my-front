...
export default function TaskAffiliation() {
    ...
    const handleRegisterPublication = async () => {
        ...
        try {
            ...
            toast.error("Échec de l'enregistrement de la publication");
            console.error("Failed to register publication:", error);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition-all duration-300 group">
                ...
                <div className="border-t border-white/5 px-4 py-3">
                    {loading ? (
                        ...
                        <div className="flex items-center gap-2">
                            <Button size="sm" onClick={handleRegisterPublication} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs h-8">
                                <Plus className="w-3.5 h-3.5 mr-1" />
                                Enregistrer
                            </Button>
                            <Button size="sm" variant="outline" onClick={handleArchive} className="bg-white/5 hover:bg-white/10 border-white/10 text-gray-300 text-xs h-8">
                                <Archive className="w-3.5 h-3.5" />
                            </Button>
                        </div>
                    ) : (
                        <div className="flex items-center gap-2">
                            <Button disabled={disabledPush} size="sm" onClick={handleCreateProduct} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-xs h-8">
                                <Plus className="w-3.5 h-3.5 mr-1" />
                                Produit
                            </Button>
                            <Button disabled={disabledPush} size="sm" onClick={handleCreateCarte} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs h-8">
