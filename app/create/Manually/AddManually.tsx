...
export default function AddManually() {
    ...
    const { selectedNiche, setSelectedNiche, selectedProduct, setSelectedProduct, updatePayloadPeluche, payloadPeluche, setAsin } = useCreateStore();
        ...
        <Card className="m-2 p-4">
            ...
            <CardContent className="flex flex-wrap gap-4 bg-gray-50 p-4 rounded">
                ...
                {selectedNiche && selectedProduct && (
                    <Inputs size={Number(payloadPeluche.size)} setSize={(value) => updatePayloadPeluche("size", Number(value))} productType={selectedProduct} setNamePokemon={(value) => updatePayloadPeluche("namePokemon", value)} namePokemon={payloadPeluche.namePokemon || ""} />
                )}
                <AddProduct />
            </CardContent>
        </Card>
