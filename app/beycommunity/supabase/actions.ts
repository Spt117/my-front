    ...
    try {
        ...
        if (adminKey && supabaseUrl) {
            ...
            if (!schemaError && schemaData && schemaData.length > 0) {
                ...
                return {
                    tables: tables.sort((a: TableInfo, b: TableInfo) => a.table_name.localeCompare(b.table_name)),
                    error: null,
                };
            }
        }
        ...
        if (foundTables.length > 0) {
            return {
                tables: foundTables.sort((a, b) => a.table_name.localeCompare(b.table_name)),
                error: null,
            };
        }
    ...
    try {
        if (!row) {
            return {
                success: false,
                row: null,
                error: "Données de la ligne manquantes",
            };
        }
...
// Récupérer une ligne spécifique d'une table
export async function fetchTableRow(
    ...
    try {
        ...
        if (idColumn) {
            query = query.eq(idColumn, rowId);
        } else {
            // Stratégie de repli sur les colonnes ID communes
            const { data: checkData, error: checkError } = await supabase.from(tableName).select("*").limit(1);
            if (checkError) throw checkError;

            const firstRow = checkData?.[0];
            const actualIdColumn = firstRow ? ["id", "uuid", "_id"].find((col) => col in firstRow) || "id" : "id";
            query = query.eq(actualIdColumn, rowId);
        }

        const { data: row, error: rowError } = await query.maybeSingle();

        if (rowError) {
            return { data: null, columns: [], error: rowError.message };
        }

        if (!row) {
            return { data: null, columns: [], error: "Ligne introuvable" };
        }

        // Extraire les colonnes à partir de la ligne
        const columns: ColumnInfo[] = [];
        ...
        return { data: null, columns: [], error: "Erreur interne du serveur" };
    }
}
